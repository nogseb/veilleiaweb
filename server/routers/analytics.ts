...
import { hasValidStatsSession } from "../statsAuth";
...
const statsProcedure = publicProcedure.use(({ ctx, next }) => {
  if (!hasValidStatsSession(ctx.req.headers.cookie)) {
    throw new TRPCError({ code: "FORBIDDEN", message: "Accès statistiques requis" });
  }
  return next();
});
...
export const analyticsRouter = router({
  ...
  dashboard: statsProcedure.input(z.object({ days: z.number().int().min(1).max(365).default(30) }).optional())
    .query(({ input }) => getAnalyticsSnapshot(input?.days ?? 30)),
  purge: statsProcedure.mutation(() => purgeExpiredAnalytics()),
  allowedChannels: statsProcedure.query(() => analyticsChannels),
});
